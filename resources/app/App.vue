            ...
            mainMenu: [
                {
                    ...
                    to: computed(() => {
                        const role = authStore.user.roles[0].id;
                        if (role === 'cs') {
                            return '/cs/dashboard';
                        } else if (role === 'kepala_teknisi') {
                ...
                },
                {
                    ...
                    showDesktop: true,
                    showMobile: true,
                    requiresAbility: getAbilitiesForRoute(['orders.list', 'orders.create', 'orders.edit']),
                    to: computed(() => {
                        const role = authStore.user.roles[0].id;
                        if (role === 'cs') {
                            return '/cs/orders/list';
                        } else if (role === 'kepala_teknisi') {
                            return '/kepala-teknisi/orders/list';
                        } else if (role === 'teknisi') {
                            return '/teknisi/orders/list';
                        } else {
                            return '/panel/orders/list';
                        }
                    }),
                    children: [
                        {
                            ...
                            showDesktop: true,
                            showMobile: true,
                            requiresAbility: getAbilitiesForRoute('orders.list'),
                            to: computed(() => {
                                const role = authStore.user.roles[0].id;
                                if (role === 'cs') {
                                    return '/cs/orders/list';
